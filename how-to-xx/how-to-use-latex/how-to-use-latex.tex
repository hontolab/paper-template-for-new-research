...
本稿は論文執筆テンプレートと同じクラスファイル（simplepaper.cls）で組版されていますので，
「ソースにこう書くと，論文ではこう出力される」という対応をそのまま確認できます．

なお，LaTeXの環境の準備やテンプレートのファイル構成については，
リポジトリ直下にある\texttt{README.md}を参照してください．
また，LaTeXの使い方については様々な書籍，ウェブサイトが解説を行っています．
より詳しい使い方についてはそちらを参考にしてください．

\section{なぜLaTeXを使うのか}
LaTeXの利点としてよく挙げられるのは「数式が美しく書ける」ことですが，
それ以上に重要なのは，\textbf{内容と見た目（スタイル）を分離できる}ことです．

Wordでは，フォントの大きさや余白，図の位置などの「見た目」を，書き手が自分で整えます．
そのため，文章を書いている途中でもレイアウト調整に意識を奪われがちです．
一方LaTeXでは，書き手は「これは章見出し」「これは図」「ここで文献を引用」という文書の構造だけを記述し，
見た目の整形はすべてLaTeXが引き受けてくれます．
おかげで，内容を練ることだけに集中できるのです．

この分離の恩恵は，論文を書き進めるほど大きくなります．
論文は一度書いて終わりではなく，指導教員のコメントを受けて何度も書き直すものです．
章の順番を入れ替えても，図を追加しても，見出しや図表の番号，本文中の参照はすべて自動で振り直されます．
投稿先が変わってフォーマットの指定が変わっても，見た目を定義するファイル（スタイルファイル）を差し替えるだけで，本文には手を入れずに済みます．
Wordでこれらを手作業でやると，手間がかかるうえに直し漏れという事故のもとになります．

最初はコマンドを覚えるのが面倒で，「Wordで書きたい」と思うかもしれません．
しかし，それは最初だけです．
本稿で扱う程度のコマンドを覚えれば，論文を書くにはほぼ十分です．
最初の一山だけ，頑張って乗り越えてください．
